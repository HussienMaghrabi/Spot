        ...
        try{
            if($checkLogin)
            {
                ...
                // insert first row in database
                    ...
                    if($data['last_login_day'] != date('Y-m-d'))
                    {
                        $data['days_count'] = $data['days_count'] +  1;
                        DB::table('login_check')->where('user_id',$userId)->update($data);
                        // check on users days
                        ...
                        else if(empty($gift_check->item_id) && empty($gift_check->gift_id) && !empty($gift_check->coins))
                        {
                            ...
                            // {
                            // }
                            DB::table('users')->where('id',$userId)->update(['coins'=> (DB::table('users')->where('id',$userId)->first()->coins =  DB::table('users')->where('id',$userId)->first()->coins + $gift_check->coins)]);
                        }        
                    }
                }
        DB::commit();
